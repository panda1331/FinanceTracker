...
| **Dependency Injection** | Все зависимости через конструкторы, DI-контейнер | Слабая связанность, тестируемость |
| **Strategy** | IPasswordHasher (BCrypt), ITokenGenerator (JWT) | Замена алгоритмов без изменения бизнес-логики |
| **DTO** | Объекты запросов/ответов в Application/DTOs | Изоляция внутренних моделей от API |
| **Facade** | Сервисы (AuthService, TransactionService и др.) | Простой интерфейс для сложной логики |
